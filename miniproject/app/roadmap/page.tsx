    ...
    <div className="space-y-8">
      ...
      <div className="text-center space-y-4">
        ...
        >
          <div className="flex items-center space-x-2">
            ...
            <span className="text-text-secondary">{roadmap.totalDuration} months</span>
          </div>
          <div className="flex items-center space-x-2">
            <div className="w-3 h-3 bg-warning rounded-full"></div>
            <span className="text-text-secondary capitalize">{roadmap.difficulty}</span>
          </div>
          <div className="flex items-center space-x-2">
            <div className="w-3 h-3 bg-primary rounded-full"></div>
            <span className="text-text-secondary">{roadmap.phases.length} phases</span>
          </div>
        </motion.div>
      </div>
      ...
      >
        <button
          ...
          className="px-8 py-4 bg-gradient-to-r from-secondary to-warning rounded-xl font-semibold text-black hover:scale-105 transition-transform duration-200 flex items-center space-x-2 shadow-lg shadow-secondary/25"
        >
          <Play size={20} />
          <span>Start Your Journey</span>
        </button>
        
        <button className="px-6 py-4 bg-slate-100 border border-slate-300 rounded-xl font-semibold text-text-primary hover:bg-slate-200 transition-colors flex items-center space-x-2">
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        <motion.div
          ...
          className="space-y-6"
        >
          <h2 className="text-2xl font-bold text-text-primary">Learning Phases</h2>
          
          {roadmap.phases.map((phase, index) => (
            <div key={phase.id} className="bg-slate-50/50 backdrop-blur-sm rounded-xl p-6 border border-slate-300">
              <div className="flex items-center space-x-3 mb-3">
                ...
                <span className="text-sm text-slate-600">({phase.duration} weeks)</span>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {phase.nodes.slice(0, 4).map((node) => (
                  <div key={node.id} className="text-sm text-slate-600 bg-slate-100/50 rounded-lg p-2">
                    {node.title}
                  </div>
                ))}
                {phase.nodes.length > 4 && (
                  <div className="text-sm text-secondary bg-slate-100/50 rounded-lg p-2 text-center">
        ...
        <motion.div
          ...
          className="space-y-6"
        >
          <div className="bg-gradient-to-br from-secondary/10 to-warning/10 backdrop-blur-sm rounded-xl p-6 border border-secondary/20">
          ...
          <div className="bg-slate-50/50 backdrop-blur-sm rounded-xl p-6 border border-slate-300">
            <h3 className="text-lg font-bold text-text-primary mb-3">Expected Outcome</h3>
            <p className="text-text-secondary">{roadmap.estimatedOutcome}</p>
          </div>

          <div className="bg-slate-50/50 backdrop-blur-sm rounded-xl p-6 border border-slate-300">
            <h3 className="text-lg font-bold text-text-primary mb-3">Quick Stats</h3>
...
export default function RoadmapPage() {
  ...
  //           <h2 className="text-xl text-text-primary mb-4">Authentication Required</h2>
  //           <p className="text-slate-600">Please sign in to access the roadmap generator.</p>
  //         </div>
  //       </div>
  //     </div>
  //   )
            ...
            <div className="flex items-center space-x-4">
              {currentView !== 'generator' && (
                <button
                  ...
                  className="p-2 bg-slate-100 border border-slate-300 rounded-lg text-text-secondary hover:text-text-primary transition-colors"
                >
                  <ArrowLeft size={20} />
                </button>
              )}
              ...
              <div className="flex space-x-2">
                {currentView === 'interactive' && (
                  <button
                    ...
                    className="px-4 py-2 bg-slate-100 border border-slate-300 rounded-lg text-text-primary hover:bg-slate-200 transition-colors"
                  >
                    3D View
                  </button>
                )}
        ...
        <AnimatePresence mode="wait">
          ...
          {currentView === '3d-view' && (
            <motion.div
              ...
              exit={{ opacity: 0 }}
              transition={{ duration: 0.5 }}
              className="bg-slate-50/50 backdrop-blur-sm rounded-2xl border border-slate-300 p-12 text-center"
            >
              <Sparkles size={48} className="mx-auto mb-4 text-secondary" />
              <h3 className="text-2xl font-bold text-text-primary mb-2">3D View Coming Soon</h3>
              <p className="text-slate-600 mb-6">The 3D roadmap visualization is under construction.</p>
              <button
                onClick={() => setCurrentView('interactive')}
                className="px-6 py-3 bg-gradient-to-r from-secondary to-warning rounded-xl font-semibold text-black"
              >
