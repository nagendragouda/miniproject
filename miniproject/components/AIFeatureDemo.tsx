    ...
    <div className="min-h-screen bg-gradient-to-br from-space-dark via-space-dark to-purple-900/20 p-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          >
            <div className="p-3 bg-gradient-to-r from-secondary to-blue-500 rounded-xl mr-4">
          ...
          <p className="text-slate-600 text-lg max-w-3xl mx-auto">
            Test and explore all AI-powered features in the FutureMatrix Platform. 
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-8">
          {filteredFeatures.map((feature) => (
            <motion.div
              ...
              className={`bg-slate-100/50 backdrop-blur-sm rounded-2xl p-6 border border-slate-300/50 cursor-pointer transition-all ${
                selectedFeature?.id === feature.id ? 'ring-2 ring-secondary' : ''
              }`}
            ...
            >
              ...
              <div className="flex items-center justify-between">
                <span className="px-2 py-1 bg-slate-200 rounded text-xs text-text-secondary">
                  {feature.category}
                </span>
        ...
        <AnimatePresence>
          {selectedFeature && (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 20 }}
              className="bg-slate-100/80 backdrop-blur-lg rounded-2xl border border-slate-300 overflow-hidden"
            >
              <div className={`bg-gradient-to-r ${selectedFeature.color} p-6`}>
                <div className="flex items-center justify-between">
                  <div className="flex items-center">
                    <selectedFeature.icon className="w-8 h-8 text-text-primary mr-3" />
                    <div>
                      <h2 className="text-2xl font-bold text-text-primary">{selectedFeature.title}</h2>
                      <p className="text-text-primary/80">{selectedFeature.description}</p>
                    </div>
                  </div>
                  <button
                    onClick={() => testAIFeature(selectedFeature)}
                    disabled={loading}
                    className="px-6 py-3 bg-white/20 backdrop-blur-sm text-text-primary rounded-lg hover:bg-white/30 transition-colors flex items-center space-x-2 disabled:opacity-50"
                  >
                    {loading ? (
                      <>
                        <motion.div
              ...
              <div className="p-6">
                ...
                <div className="mt-6 p-4 bg-slate-50/50 rounded-lg">
                  <h4 className="text-text-primary font-medium mb-2">API Endpoint</h4>
                  <code className="text-secondary text-sm bg-slate-100 px-3 py-1 rounded">
        ...
        >
          <div className="bg-slate-100/50 backdrop-blur-sm rounded-xl p-4 text-center">
            <div className="text-2xl font-bold text-secondary mb-1">{aiFeatures.length}</div>
            <div className="text-slate-600 text-sm">AI Features</div>
          </div>
          <div className="bg-slate-100/50 backdrop-blur-sm rounded-xl p-4 text-center">
          ...
          <div className="bg-slate-100/50 backdrop-blur-sm rounded-xl p-4 text-center">
            ...
            <div className="text-slate-600 text-sm">AI Providers</div>
          </div>
          <div className="bg-slate-100/50 backdrop-blur-sm rounded-xl p-4 text-center">
