  ...
  return (
    <group position={position}>
      ...
      {hovered && (
        ...
        >
          <div className="bg-black/90 backdrop-blur-sm text-text-primary p-3 rounded-lg border border-secondary/30 max-w-xs">
            <div className="font-semibold text-sm mb-1">{node.title}</div>
            {node.description && (
              <div className="text-xs text-text-secondary mb-2">{node.description.slice(0, 80)}...</div>
            )}
...
export default function Enhanced3DRoadmapVisualization({ roadmap }: Enhanced3DRoadmapProps) {
  ...
  if (isLoading) {
    return (
      <div className="w-full h-full bg-gradient-to-br from-space-dark via-purple-900/20 to-space-dark rounded-xl border border-secondary/20 flex items-center justify-center relative overflow-hidden">
        ...
        <div className="relative z-10 text-center">
          ...
          >
            ...
            <Target className="w-8 h-8 text-secondary absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2" />
          </motion.div>
          <motion.p 
  ...
  if (error) {
    return (
      <div className="w-full h-full bg-background rounded-xl border border-red-500/20 flex items-center justify-center">
    ...
    <div className="relative w-full h-full rounded-xl overflow-hidden bg-gradient-to-br from-space-dark to-purple-900/20">
      ...
      <AnimatePresence>
        {showControls && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 20 }}
            className="absolute top-4 left-4 bg-black/80 backdrop-blur-sm rounded-lg p-4 border border-secondary/20"
          >
            <h4 className="text-text-primary font-semibold mb-3 text-sm flex items-center">
            ...
            <div className="space-y-3 text-sm">
              {/* Camera Mode */}
              <div>
                <label className="text-text-secondary block mb-2">Camera Mode</label>
                <select 
                  value={cameraMode}
                  onChange={(e) => setCameraMode(e.target.value as any)}
                  className="w-full bg-black/50 border border-white/20 rounded px-2 py-1 text-text-primary text-xs"
                >
              ...
              {/* Filter */}
              <div>
                <label className="text-text-secondary block mb-2">Show Nodes</label>
                <select 
            ...
            <div className="flex items-center justify-between mt-4 pt-3 border-t border-white/10">
              <span className="text-xs text-slate-600">Mouse: Rotate • Scroll: Zoom</span>
              <button 
      ...
      {!showControls && (
        <button
          ...
          className="absolute top-4 left-4 p-2 bg-black/80 backdrop-blur-sm rounded-lg border border-secondary/20 text-slate-600 hover:text-text-primary transition-colors"
        >
      ...
      {/* Legend */}
      <div className="absolute bottom-4 right-4 bg-black/80 backdrop-blur-sm rounded-lg p-4 border border-secondary/20">
        <h5 className="text-text-primary font-semibold mb-2 text-sm">Legend</h5>
      ...
      {/* Performance Info */}
      <div className="absolute top-4 right-4 bg-black/60 backdrop-blur-sm rounded px-3 py-1 text-xs text-slate-600">
