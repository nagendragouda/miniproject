...
function loadGoogleMaps(apiKey: string): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    ...
    if (existing) {
      // Script is already being loaded or loaded
    ...
    // Only create and append script if it doesn't already exist
...
export default function GoogleCollegesMap({ colleges, selectedCollegeId, apiKeyOverride, onMapError }: Props) {
  ...
  useEffect(() => {
    ...
    if (!apiKey) {
      onMapError?.('Google Maps API key is missing')
      return
    }
    ...
    loadGoogleMaps(apiKey)
      ...
      .catch((err) => {
        ...
        onMapError?.(err?.message || 'Google Maps failed to initialize')
      })
  ...
  }, [colleges, selectedCollegeId, apiKeyOverride, onMapError])
