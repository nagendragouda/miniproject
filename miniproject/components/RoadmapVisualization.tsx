      ...
      {phases.map((phase, phaseIndex) => (
        <div key={phase.id} className="relative">
          ...
          {phaseIndex < phases.length - 1 && (
            <div className="absolute left-6 top-16 w-0.5 h-full bg-gradient-to-b from-secondary to-primary opacity-30" />
          )}
          ...
          <div className="flex items-start space-x-4 mb-4">
            <div className={`w-12 h-12 rounded-full flex items-center justify-center border-2 ${
              phase.completed 
                ...
                : 'border-secondary text-secondary bg-black/30'
            }`}>
              <Target className="w-6 h-6" />
            </div>
            <div className="flex-1">
              <h3 className="text-xl font-semibold text-text-primary mb-2">{phase.title}</h3>
              <p className="text-text-secondary mb-2">{phase.description}</p>
          ...
          <div className="ml-16 space-y-3">
            {phase.milestones?.map((milestone, milestoneIndex) => (
              <motion.div
                ...
                className={`bg-black/30 backdrop-blur-sm border rounded-lg p-4 cursor-pointer hover:border-secondary/50 transition-colors ${
                  selectedNode?.id === milestone.id ? 'border-secondary' : 'border-white/10'
                }`}
                ...
                {milestone.skills?.length > 0 && (
                  <div className="flex flex-wrap gap-1 mb-2">
                    ...
                    {milestone.skills.length > 3 && (
                      <span className="px-2 py-1 bg-gray-500/20 text-slate-600 text-xs rounded">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-3">
              <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                phase.completed ? 'bg-green-500' : 'bg-secondary'
              }`}>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-3">
            {phase.milestones?.map((milestone) => (
              <div
                key={milestone.id}
                className={`border rounded-lg p-3 cursor-pointer transition-colors hover:border-secondary/50 ${
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
              view === 'phases' 
                ...
                : 'bg-black/30 text-text-secondary hover:text-text-primary'
            }`}
          >
            Phases View
          </button>
        ...
        <div className="lg:col-span-1">
          <div className="bg-black/30 backdrop-blur-sm border border-white/10 rounded-xl p-6 sticky top-4">
            {selectedNode ? (
              <div>
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-lg font-semibold text-text-primary">{selectedNode.data.title}</h3>
                  <button
                    onClick={() => setSelectedNode(null)}
                    className="text-slate-600 hover:text-text-primary"
                  >
                    ✕
                  </button>
                </div>

                <p className="text-text-secondary mb-4">{selectedNode.data.description}</p>

                {selectedNode.data.skills?.length > 0 && (
                  <div className="mb-4">
                    <h4 className="text-sm font-medium text-text-primary mb-2">Skills to Develop</h4>
                    ...
                    <div className="space-y-2">
                      {selectedNode.data.resources.slice(0, 3).map((resource: Resource, index: number) => (
                        <div key={index} className="bg-black/20 rounded-lg p-3">
                          ...
                          <div className="text-xs text-slate-600 mb-2">{resource.type} • {resource.duration}</div>
                ...
                {selectedNode.data.deliverables?.length > 0 && (
                  <div>
                    <h4 className="text-sm font-medium text-text-primary mb-2">Deliverables</h4>
              ...
              <div className="text-center text-slate-600">
                ...
                <h3 className="font-medium text-text-primary mb-2">Roadmap Overview</h3>
                <p className="text-sm mb-4">{roadmap.description}</p>
                
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between">
                    <span>Total Phases</span>
                    <span className="text-secondary">{phases.length}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Total Milestones</span>
                    <span className="text-secondary">{totalMilestones}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Progress</span>
                    <span className="text-secondary">{Math.round((completedMilestones / totalMilestones) * 100) || 0}%</span>
                  </div>
                </div>

                <p className="text-xs text-slate-500 mt-4">Click on any phase or milestone to see details</p>
              </div>
            )}
          </div>
          ...
          {roadmap.ai_recommendations && (
            <div className="mt-6 bg-black/30 backdrop-blur-sm border border-white/10 rounded-xl p-6">
              ...
              {roadmap.ai_recommendations.colleges?.length > 0 && (
                <div className="mb-4">
                  <h4 className="text-sm font-medium text-text-primary mb-2">Recommended Colleges</h4>
                  <div className="space-y-2">
                    {roadmap.ai_recommendations.colleges.slice(0, 2).map((college, index) => (
                      <div key={index} className="bg-black/20 rounded-lg p-3">
                        <div className="text-sm font-medium text-text-primary">{college.name}</div>
                        <div className="text-xs text-slate-600">{college.location} • {college.type}</div>
                        <div className="text-xs text-text-secondary mt-1">{college.program}</div>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {roadmap.timeline && (
                <div>
                  <h4 className="text-sm font-medium text-text-primary mb-2">Timeline Goals</h4>
                  <div className="space-y-2 text-xs">
                    <div>
                      <span className="text-slate-600">Short-term:</span>
                      <span className="text-text-secondary ml-2">{roadmap.timeline.short_term}</span>
                    </div>
                    <div>
                      <span className="text-slate-600">Long-term:</span>
                      <span className="text-text-secondary ml-2">{roadmap.timeline.long_term}</span>
                    </div>
                  </div>
                </div>
              )}
