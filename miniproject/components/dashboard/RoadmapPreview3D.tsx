...
export default function RoadmapPreview3D({ userId }: { userId?: string }) {
  ...
  useEffect(() => {
    const fetchRoadmapData = async () => {
      try {
        if (!userId) {
          // Provide default roadmap when no userId
          setRoadmapData({
            phases: [
              { id: '1', title: 'Foundation', status: 'completed', skills: [
                { id: 's1', name: 'HTML', level: 'beginner' },
                { id: 's2', name: 'CSS', level: 'beginner' }
              ]},
              { id: '2', title: 'Intermediate', status: 'in_progress', skills: [
                { id: 's3', name: 'JavaScript', level: 'intermediate' },
                { id: 's4', name: 'React', level: 'intermediate' }
              ]},
              { id: '3', title: 'Advanced', status: 'locked', skills: [
                { id: 's5', name: 'Node.js', level: 'advanced' },
                { id: 's6', name: 'Full Stack', level: 'advanced' }
              ]}
            ]
          })
          setLoading(false)
  ...
  if (webglError) {
    return (
      <div className="relative h-64 bg-white-900/20 to-black/20 rounded-xl overflow-hidden flex items-center justify-center">
        <div className="text-center text-slate-600">
          <div className="text-4xl mb-2">🗺️</div>
          <div className="text-sm">3D Preview Unavailable</div>
          <div className="text-xs text-slate-500">WebGL not supported</div>
        </div>
      </div>
    )
  }
    ...
    return (
      <div className="relative h-64 bg-white-900/20 to-black/20 rounded-xl overflow-hidden flex items-center justify-center">
        <div className="text-center text-slate-600">
          ...
          <div className="text-lg font-semibold text-text-primary mb-2">Career Roadmap</div>
          <div className="text-sm text-slate-600 mb-4 px-4">
            Build your personalized career roadmap and track your progress towards your goals
          </div>
        </div>
      </div>
    )
  ...
  return (
    <div className="relative h-64 bg-white-900/20 to-black/20 rounded-xl overflow-hidden" style={{ marginTop: '0px' }}>
      <Canvas
        ref={canvasRef}
