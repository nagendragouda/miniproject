...
export default function SkillProgress3D({ skills }: { skills?: Skill[] }) {
  // Default mock skills if none provided
  const displaySkills = skills || [
    { name: 'JavaScript', current: 85, target: 95, category: 'Programming' },
    { name: 'React', current: 80, target: 90, category: 'Frontend' },
    { name: 'Node.js', current: 75, target: 90, category: 'Backend' },
    { name: 'Problem Solving', current: 88, target: 95, category: 'Soft Skills' },
  ]

  return (
    <div className="h-48 bg-white-900/20 to-black/20 rounded-xl overflow-hidden">
      <Canvas
      ...
      <div className="absolute bottom-2 left-4 right-4">
        <div className="flex justify-between items-center text-xs text-slate-600">
          ...
          <span className="text-secondary font-semibold">
            {Math.round(displaySkills.reduce((sum, skill) => sum + skill.current, 0) / displaySkills.length)}%
          </span>
        </div>
      </div>
    </div>
