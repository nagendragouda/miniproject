...
test.describe('FutureMatrix - Homepage', () => {
  ...
  test('should load homepage with correct title and meta information', async ({ page }) => {
    // Check page title
    await expect(page).toHaveTitle('FutureMatrix | Your Personalized Career & College Guide');
  ...
  test('should display navigation with logo and menu items', async ({ page }) => {
    ...
    await expect(page.getByRole('link', { name: 'FM FutureMatrix' })).toBeVisible();
