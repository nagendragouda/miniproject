...
test.describe('Subscription Workflows - Payment & Feature Access', () => {
  ...
  test('Should validate API access control', async ({ page }) => {
    // Test subscription API endpoint access control (should require auth)
    const subResponse = await page.request.get('/api/subscription')
    // Response should be either 401 (unauthorized) or 200 (allow guest access)
  ...
  test('Should validate chatbot subscription model integration', async ({ page }) => {
    ...
    await expect(page.getByText('FutureMatrix AI')).toBeVisible()
